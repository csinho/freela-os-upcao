...
import { downloadOrcamentoPdf, openOrcamentoPdf } from "@/lib/pdfmake-client";
...
export function PDFPreview({ orcamento, empresa, cliente }: PdfProps) {
  ...
  useEffect(() => {
    ...
    setError(null);
    setUrl(null);
    openOrcamentoPdf(orcamento, empresa, cliente)
      .then((u) => {
        revoked = u;
        setUrl(u);
      })
  ...
  if (!url) {
    return (
      <div className="p-8 text-center text-sm text-muted-foreground">Gerando PDF…</div>
    );
  }
  return (
    <iframe
...
export function DownloadBtn({ orcamento, empresa, cliente }: PdfProps) {
  ...
  const handleDownload = async () => {
    setLoading(true);
    try {
      await downloadOrcamentoPdf(orcamento, empresa, cliente);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Button variant="outline" disabled={loading} onClick={handleDownload}>
