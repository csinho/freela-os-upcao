...
export function getBillingUiState(empresa: EmpresaBillingRow, now = new Date().toISOString()): BillingUiState {
...
export function billingBlocksMutation(state: BillingUiState): boolean {
  return state.needsPayment || state.billingStatus === "pendente" || state.billingStatus === "inadimplente";
}
